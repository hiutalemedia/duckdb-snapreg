...
static bool validate_point(double feat, double target, const Rational &slope, const Rational &intercept) {
	if (std::abs(feat - std::round(feat)) < 1e-9) {
		int64_t fi = (int64_t)std::llround(feat);
		if (!is_integer_result(fi, slope, intercept))
			return false;
		return integer_result(fi, slope, intercept) == (int64_t)std::llround(target);
	}
	double pred = slope.to_double() * feat + intercept.to_double();
	return std::abs(pred - std::round(pred)) < 1e-9 && std::abs(pred - target) < 0.5;
}
...
static void EqAggUpdate(Vector inputs[], AggregateInputData &, idx_t input_count, Vector &state_vec, idx_t count) {
	...
	for (idx_t i = 0; i < count; i++) {
		...
		if (!target_fmt.validity.RowIsValid(ti) || !feat_fmt.validity.RowIsValid(fi))
			continue;

		auto &s = *reinterpret_cast<EquationAggState *>(states[i]);
		if (den_fmt.validity.RowIsValid(di))
			s.max_den = max_dens[di];
		s.AddPoint(feats[fi], targets[ti], ex_fmt.validity.RowIsValid(ei) ? ex_ids[ei] : 0,
		           row_fmt.validity.RowIsValid(ri) ? row_idxs[ri] : (int32_t)i);
	}
}

static void EqAggCombine(Vector &src_vec, Vector &tgt_vec, AggregateInputData &, idx_t count) {
...
static void EqAggFinalize(Vector &state_vec, AggregateInputData &, Vector &result, idx_t count, idx_t offset) {
	auto *states = FlatVector::GetData<data_ptr_t>(state_vec);
	...
	for (idx_t i = 0; i < count; i++) {
		...
		for (int64_t j = 0; j < s.n; j++) {
			...
			if (std::abs(feat - std::round(feat)) < 1e-9) {
				...
				ok = (std::abs(pred - std::round(pred)) < 1e-9) && (std::abs(pred - (*s.ys)[j]) < 0.5);
			}
...
void RegisterEquationAgg(ExtensionLoader &loader) {
	...
	AggregateFunction agg(
	    "equation_agg",
	    /* args */
	    {LogicalType::DOUBLE, LogicalType::DOUBLE, LogicalType::INTEGER, LogicalType::INTEGER, LogicalType::INTEGER},
