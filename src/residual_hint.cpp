...
static void ResidualHintFunction(DataChunk &args, ExpressionState &, Vector &result) {
	...
	for (idx_t i = 0; i < count; i++) {
		...
		for (double v : vals)
			rounded.push_back((int64_t)std::llround(v));
		std::sort(rounded.begin(), rounded.end());
		auto n_dist = (int32_t)std::distance(rounded.begin(), std::unique(rounded.begin(), rounded.end()));
...
void RegisterResidualHint(ExtensionLoader &loader) {
	...
	ScalarFunction fn("residual_hint", {LogicalType::LIST(LogicalType::DOUBLE)}, result_type, ResidualHintFunction);
